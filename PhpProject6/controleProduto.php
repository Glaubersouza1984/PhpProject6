...
if(!empty($_POST)){
    ...
    if(isset($_POST['produto']) && isset($_POST['descricao']) && isset($_POST['quantidade'])
            && isset($_POST['valor'])){
        if (!empty($_POST['produto'])&& !empty($_POST['descricao']) && !empty($_POST['quantidade'])
                && !empty($_POST['valor'])){
    ...
    if($operacao=="inserir"){
        $produto = new Produto();
        $resultado = $produto->adicinarProduto($dados);
        if($resultado) header('location: index.php?resposta=Sucesso&mensagem=Produto inserido com sucesso');
        else
            header('location: index.php?resposta=Erro&mensagem=Ocorreu um erro ao inserir o produto');
        }else if($operacao=="alterar" && isset($_POST['id']) && !empty($_POST['id'])){
            $id = $_POST["id"];
            $produto = new Produto();
            $resultado = $produto->alterarProduto($id, $dados);
                ...
                else header('location:  index.php?resposta=Erro&mensagem=Ocorreu um erro ao alterar o produto');
        }else{            
        }
    }elseif(!empty($_GET)){
        ...
        if($operacao=="excluir" && isset($_GET['id']) && !empty($_GET['id'])){
            ...
            else header ('location: index.php?resposta=Erro&mensagem=Ocorreu um erro ao excluir o produto !');
        }
    }
